...
def make_network_public(network_id, username, password):
    url = "http://54.148.42.155/rest/network/" + network_id + "/summary"
...
for network in networks:
    if network['visibility'] != 'PUBLIC':
        status_code = make_network_public(network['externalId'], arg.username, arg.password)
        if status_code == 204:
            changes += 1
